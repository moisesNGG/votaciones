...
// Crear el contexto
const VotacionContext = createContext<VotacionContextType | undefined>(
  undefined
);
...
export const VotacionProvider: React.FC<VotacionProviderProps> = ({
  children,
}) => {
  ...
  const cargarDatos = () => {
    ...
    try {
      ...
      console.log(
        "VotacionContext: candidatos con imágenes:",
        candidatosConImagenes
      );
  ...
  const emitirVoto = (candidatoId: number): boolean => {
    ...
    if (exito) {
      ...
      const candidatoVotado = listaDeCandidatos.find(
        (c) => c.id === candidatoId
      );
