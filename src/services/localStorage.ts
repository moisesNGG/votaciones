...
export const obtenerDatos = (): BaseDatosLocal => {
  try {
    ...
    console.log('obtenerDatos: No hay datos en localStorage, devolviendo datos iniciales');
...
export const verificarSiYaVoto = (cedula: string): boolean => {
  ...
  return datos.usuarios.some(usuario => usuario.cedula === cedula && usuario.yaVoto);
};

// Función para registrar un nuevo voto
export const registrarVoto = (usuario: Usuario, candidatoId: number): boolean => {
  try {
    const datos = obtenerDatos();
    ...
    const usuarioIndex = datos.usuarios.findIndex(u => u.cedula === usuario.cedula);
    ...
    const candidatoIndex = datos.candidatos.findIndex(c => c.id === candidatoId);
    if (candidatoIndex >= 0) {
